...
def one_epoch(model, criterion, opt, config, dataloader, device, epoch, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None):
    ...
    with grad_context():
        ...
        for iter_i, batch in iterator:
            with autograd.detect_anomaly():
                ...
                # 下面的模型输出这么多变量，是为了？记录各种误差
                if model_type == "alg" or model_type == "ransac":
                    keypoints_3d_pred, keypoints_2d_pred, heatmaps_pred, confidences_pred = \
                        model(images_batch, proj_matricies_batch, batch)
                elif model_type == "vol":
                    keypoints_3d_pred, heatmaps_pred, volumes_pred, confidences_pred, cuboids_pred, coord_volumes_pred, base_points_pred \
                        = model(images_batch, proj_matricies_batch, batch)
                ...
                if n_views == 1:
                    ...
                    #将所有的坐标转换为以基准点为0点的坐标, 为什么多视角时坐标就不以基准点转换
                ...
                # volumetric ce loss, 如果使用volumetric ce loss而不是mean square error loss
    ...
    # calculate evaluation metrics, 记录官方的度量信息并存储结果
    if master:
        if not is_train:
